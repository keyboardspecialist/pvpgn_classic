...
namespace pvpgn
{
	...
	namespace d2dbs
	{
		...
		static unsigned int dbs_packet_savedata_charsave(t_d2dbs_connection* conn, char * AccountName, char * CharName, char * data, unsigned int datalen)
		{
			...
			if (datalen < D2CHARSAVE_VERSION_OFFSET + sizeof(bn_int)) {
				eventlog(eventlog_level_error, __FUNCTION__, "charsave is too short to contain a version - discarding charsave");
				return 0;
			}

			version = bn_int_get((bn_basic*)&data[D2CHARSAVE_VERSION_OFFSET]);
			if (version >= D2CHARSAVE_CHECKSUM_MIN_VERSION) {
				if (datalen < D2CHARSAVE_CHECKSUM_OFFSET + sizeof(bn_int)) {
					eventlog(eventlog_level_error, __FUNCTION__, "charsave is too short to contain a checksum - discarding charsave");
					return 0;
				}
