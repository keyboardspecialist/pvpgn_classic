...
namespace pvpgn
{
	static_assert(sizeof(t_server_realmjoinreply_100) == 60 &&
	              offsetof(t_server_realmjoinreply_100, bncs_addr1) == 12 &&
	              offsetof(t_server_realmjoinreply_100, secret_hash) == 40,
	              "Diablo II 1.00 realm join reply layout changed");
	...
	namespace bnetd
	{
		...
		static int _client_realmjoinreq(t_connection * c, t_packet const *const packet, bool legacy_d2)
		{
			...
			if (packet_get_size(packet) < sizeof(t_client_realmjoinreq_109)) {
				eventlog(eventlog_level_error, __FUNCTION__, "[{}] got bad realm join packet (expected {} bytes, got {})", conn_get_socket(c), sizeof(t_client_realmjoinreq_109), packet_get_size(packet));
				return -1;
			}

			{
				...
				if (!(realmname = packet_get_str_const(packet, sizeof(t_client_realmjoinreq_109), MAX_REALMNAME_LEN))) {
					eventlog(eventlog_level_error, __FUNCTION__, "[{}] got bad realm join request (missing or too long realmname)", conn_get_socket(c));
					return -1;
				}
						...
						if (hash_set_str(&passhash, pass_str) == 0) {
							...
							if ((rpacket = packet_create(packet_class_bnet))) {
								if (legacy_d2) {
									...
									bn_int_set(&rpacket->u.server_realmjoinreply_100.u1, 0x0);
									bn_short_set(&rpacket->u.server_realmjoinreply_100.u3, 0x0);
									bn_int_set(&rpacket->u.server_realmjoinreply_100.bncs_addr1, 0x0);
									bn_int_set(&rpacket->u.server_realmjoinreply_100.sessionnum, conn_get_sessionnum(c));
								}
								else {
								...
								if (legacy_d2) {
									...
									hash_to_bnhash((t_hash const *)&secret_hash, rpacket->u.server_realmjoinreply_100.secret_hash);
								}
								else {
		...
		static unsigned int append_legacy_d2_charlist(t_connection * c, t_packet * packet)
		{
			...
			if (!realm) {
				LIST_TRAVERSE_CONST(realmlist(),curr) {
					t_realm const * candidate=(t_realm const *)elem_get_data(curr);
					if (realm_get_active(candidate)) {
						realm=candidate;
						break;
					}
				}
			...
			directory=std::string(prefs_get_d2cs_charinfo_dir())+"/"+conn_get_username(c);
			count=0;
			try {
				Directory dir(directory.c_str());
				while (count<max_characters && (charname=dir.read())) {
					t_d2charinfo_file charinfo;
					char portrait[D2CHARINFO_PORTRAIT_STORAGE_SIZE];
					unsigned char const * source;
					void const * end;
					std::size_t length;
					std::ifstream file((directory+"/"+charname).c_str(),std::ios::in|std::ios::binary);

					if (!file.read((char *)&charinfo,sizeof(charinfo)) ||
						bn_int_get(charinfo.header.magicword)!=D2CHARINFO_MAGICWORD ||
						bn_int_get(charinfo.header.version)!=D2CHARINFO_VERSION)
						continue;
					source=(unsigned char const *)&charinfo.portrait;
					end=std::memchr(source,'\0',sizeof(charinfo.portrait));
					length=end ? (unsigned char const *)end-source : sizeof(charinfo.portrait);
					std::memset(portrait,0,sizeof(portrait));
					if (length>=D2CHARINFO_PORTRAIT_LEGACY_BASE_SIZE-1) {
						std::memcpy(portrait,source,
							length<sizeof(portrait)-1 ? length : sizeof(portrait)-1);
					}
					else {
						portrait[0]=(char)0x83;
						portrait[1]=(char)0x80;
						std::memset(portrait+2,0xFF,16);
						std::memcpy(portrait+2,source+2,11);
						portrait[18]=source[13];
						std::memset(portrait+19,0xFF,16);
						std::memcpy(portrait+19,source+14,11);
					...
					packet_append_string(packet,charname);
					packet_append_string(packet,portrait);
					count++;
				}
			}
