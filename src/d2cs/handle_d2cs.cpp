...
namespace pvpgn
{
	static_assert(sizeof(t_client_d2cs_loginreq_100) == 51 &&
	              offsetof(t_client_d2cs_loginreq_100, sessionnum) == 15 &&
...
/* 0x01 */ { sizeof(t_client_d2cs_loginreq_100),    conn_state_connected,                     on_client_loginreq        },
...
{
	t_packet	* bnpacket, * authpacket, * normalized_packet;
	char const	* account;
	t_sq		* sq;
	...
	authpacket=packet;
	account=packet_get_size(packet)>sizeof(t_client_d2cs_loginreq) ? packet_get_str_const(packet,sizeof(t_client_d2cs_loginreq),MAX_CHARNAME_LEN) : NULL;
	if (!account || sizeof(t_client_d2cs_loginreq)+std::strlen(account)+1 != packet_get_size(packet) || d2char_check_acctname(account)<0) {
		account=packet_get_str_const(packet,sizeof(t_client_d2cs_loginreq_100),MAX_CHARNAME_LEN);
		legacy_100=true;
	}
	if (legacy_100 && (!account || sizeof(t_client_d2cs_loginreq_100)+std::strlen(account)+1 != packet_get_size(packet) || d2char_check_acctname(account)<0)) {
		eventlog(eventlog_level_error,__FUNCTION__,"got bad account name");
		return -1;
	}
	conn_set_legacy_100(c,legacy_100);
	if (!bnetd_conn()) {
		eventlog(eventlog_level_warn,__FUNCTION__,"d2cs is offline with bnetd, login request will be rejected");
		return -1;
	}
	if (legacy_100) {
		if (!(normalized_packet=packet_create(packet_class_d2cs)))
			return -1;
		packet_set_size(normalized_packet,sizeof(t_client_d2cs_loginreq));
		packet_set_type(normalized_packet,CLIENT_D2CS_LOGINREQ);
		std::memcpy(normalized_packet->u.client_d2cs_loginreq.seqno,packet->u.client_d2cs_loginreq_100.seqno,sizeof(bn_int));
		std::memcpy(normalized_packet->u.client_d2cs_loginreq.u1,packet->u.client_d2cs_loginreq_100.u1,sizeof(bn_int));
		std::memcpy(normalized_packet->u.client_d2cs_loginreq.bncs_addr1,packet->u.client_d2cs_loginreq_100.bncs_addr1,sizeof(bn_int));
		std::memcpy(normalized_packet->u.client_d2cs_loginreq.sessionnum,packet->u.client_d2cs_loginreq_100.sessionnum,sizeof(bn_int));
		std::memcpy(normalized_packet->u.client_d2cs_loginreq.sessionkey,packet->u.client_d2cs_loginreq_100.sessionkey,sizeof(bn_int));
		std::memcpy(normalized_packet->u.client_d2cs_loginreq.cdkey_id,packet->u.client_d2cs_loginreq_100.cdkey_id,sizeof(bn_int));
		std::memcpy(normalized_packet->u.client_d2cs_loginreq.u5,packet->u.client_d2cs_loginreq_100.u5,sizeof(bn_int));
		bn_int_set(&normalized_packet->u.client_d2cs_loginreq.clienttag,0);
		bn_int_set(&normalized_packet->u.client_d2cs_loginreq.bnversion,0);
		bn_int_set(&normalized_packet->u.client_d2cs_loginreq.bncs_addr2,0);
		bn_int_set(&normalized_packet->u.client_d2cs_loginreq.u6,0);
		std::memcpy(normalized_packet->u.client_d2cs_loginreq.secret_hash,packet->u.client_d2cs_loginreq_100.secret_hash,sizeof(normalized_packet->u.client_d2cs_loginreq.secret_hash));
		if (packet_append_string(normalized_packet,account)<0) {
			packet_del_ref(normalized_packet);
			return -1;
		}
		authpacket=normalized_packet;
	}
	...
	if ((bnpacket=packet_create(packet_class_d2cs_bnetd))) {
		if ((sq=sq_create(d2cs_conn_get_sessionnum(c),authpacket,0))) {
			...
			bn_int_set(&bnpacket->u.d2cs_bnetd_accountloginreq.seqno,
				bn_int_get(authpacket->u.client_d2cs_loginreq.seqno));
			bn_int_set(&bnpacket->u.d2cs_bnetd_accountloginreq.sessionkey,
				bn_int_get(authpacket->u.client_d2cs_loginreq.sessionkey));
			...
			std::memcpy(bnpacket->u.d2cs_bnetd_accountloginreq.secret_hash,
				authpacket->u.client_d2cs_loginreq.secret_hash,
...
static int on_client_createcharreq(t_connection * c, t_packet * packet)
{
	...
	if (d2char_create(account,charname,chclass,status,conn_get_legacy_100(c),conn_get_patch_tag(c))<0) {
		...
		if ((bnpacket=packet_create(packet_class_d2cs_bnetd))) {
			if ((sq=sq_create(d2cs_conn_get_sessionnum(c),packet,0))) {
				...
				d2char_portrait_render(&data.portrait,conn_get_legacy_100(c),portrait);
				packet_append_string(bnpacket,portrait);
				conn_push_outqueue(bnetd_conn(),bnpacket);
			}
			packet_del_ref(bnpacket);
		}
...
static int on_client_charlistreq_100(t_connection * c, t_packet * packet)
{
	if (!packet)
		return -1;
	return d2cs_send_client_charlist_100(c);
}

extern int d2cs_send_client_charlist_100(t_connection * c)
{
	static unsigned int const max_legacy_characters = 8;
	static unsigned int const assembled_header_size = 8;
	static unsigned int const record_header_size = 12;
	t_packet * rpacket;
	char const * account;
	char const * charname;
	char * path;
	t_d2charinfo_file charinfo;
	unsigned char assembled[assembled_header_size +
		max_legacy_characters * (record_header_size + MAX_CHARNAME_LEN)];
	unsigned int count;
	unsigned int maxchar;
	unsigned int offset;
	unsigned int expire_time;
	unsigned int expiration_unix;
	unsigned int status;
	unsigned int summary;
	unsigned long long expiration_filetime;
	bool retry;
	...
	if (maxchar>max_legacy_characters)
		maxchar=max_legacy_characters;
	count=0;
	offset=assembled_header_size;
	expire_time=prefs_get_char_expire_time();
	retry=true;
	while (retry) {
		try {
			Directory dir(path);
			while (count<maxchar && (charname=dir.read())) {
				if (d2charinfo_load(account,charname,&charinfo)<0) {
					eventlog(eventlog_level_error,__FUNCTION__,
						"error loading charinfo for {}(*{})",charname,account);
					continue;
				}

				expiration_unix=0x7FFFFFFF;
				if (expire_time && bn_int_get(charinfo.header.last_time)<=0xFFFFFFFF-expire_time)
					expiration_unix=bn_int_get(charinfo.header.last_time)+expire_time;
				expiration_filetime=((unsigned long long)expiration_unix+11644473600ULL)*10000000ULL;
				bn_int_set((bn_int*)(assembled+offset),(unsigned int)expiration_filetime);
				bn_int_set((bn_int*)(assembled+offset+4),(unsigned int)(expiration_filetime>>32));

				status=bn_int_get(charinfo.summary.charstatus);
				summary=bn_int_get(charinfo.summary.charclass)&0x0F;
				if (charstatus_get_hardcore(status)) summary|=0x10;
				if (charstatus_get_dead(status)) summary|=0x20;
				summary|=(bn_int_get(charinfo.summary.charlevel)&0xFF)<<16;
				bn_int_set((bn_int*)(assembled+offset+8),summary);
				std::strncpy((char*)assembled+offset+record_header_size,charname,MAX_CHARNAME_LEN-1);
				offset+=record_header_size+MAX_CHARNAME_LEN;
				count++;
			}
			retry=false;
		}
	...
	xfree(path);
	bn_int_set((bn_int*)assembled,count);
	bn_int_set((bn_int*)(assembled+4),MAX_CHARNAME_LEN);

	if ((rpacket=packet_create(packet_class_d2cs))) {
		packet_set_size(rpacket,sizeof(t_d2cs_client_charlistreply_100));
		packet_set_type(rpacket,D2CS_CLIENT_CHARLISTREPLY_100);
		bn_byte_set(&rpacket->u.d2cs_client_charlistreply_100.reserved,0);
		bn_short_set(&rpacket->u.d2cs_client_charlistreply_100.total_length,offset);
		bn_short_set(&rpacket->u.d2cs_client_charlistreply_100.current_length,offset);
		bn_short_set(&rpacket->u.d2cs_client_charlistreply_100.continuation,0);
		packet_append_data(rpacket,assembled,offset);
		conn_push_outqueue(c,rpacket);
		packet_del_ref(rpacket);
	}
			...
			try {
				...
				if (!std::strcmp(charlist_sort_order, "ASC"))
				{
					...
					elist_for_each_safe(curr, &charlist_head, safe)
					{
						...
						d2char_portrait_render(&ccharlist->charinfo->portrait,conn_get_legacy_100(c),portrait);
