...
namespace pvpgn
{
	...
	namespace d2cs
	{
		...
		static bool d2char_portrait_is_legacy(t_d2charinfo_portrait const * portrait)
		{
			unsigned char const * data = (unsigned char const *)portrait;
			void const * end = std::memchr(data, '\0', sizeof(*portrait));
			std::size_t length = end ? (unsigned char const *)end-data : sizeof(*portrait);

			return length >= D2CHARINFO_PORTRAIT_LEGACY_BASE_SIZE-1;
		}
			...
			unsigned char chclass, unsigned short status)
		{
			...
			std::memset(chardata->portrait.legacy_tail, 0, sizeof(chardata->portrait.legacy_tail));
		...
		extern int d2charinfo_load(char const * account, char const * charname, t_d2charinfo_file * data)
		{
			...
			if (!(charstatus_get_ladder(bn_int_get(data->summary.charstatus)))) {
				if (!d2char_portrait_is_legacy(&data->portrait))
		...
		extern int d2char_portrait_init(t_d2charinfo_portrait * portrait)
		{
			...
			end = std::memchr(p, '\0', sizeof(t_d2charinfo_portrait));
			length = end ? (unsigned char *)end-p : sizeof(t_d2charinfo_portrait);
			if (length >= D2CHARINFO_PORTRAIT_LEGACY_BASE_SIZE-1) {
				p[sizeof(t_d2charinfo_portrait)-1] = '\0';
				return 0;
			}
			for (i = 0; i < D2CHARINFO_PORTRAIT_MODERN_SIZE-1; i++) {
				if (!p[i]) p[i] = D2CHARINFO_PORTRAIT_PADBYTE;
			}
			p[D2CHARINFO_PORTRAIT_MODERN_SIZE-1] = '\0';
			return 0;
		}


		extern int d2char_portrait_render(t_d2charinfo_portrait const * portrait,
				unsigned int legacy_100, char output[D2CHARINFO_PORTRAIT_STORAGE_SIZE])
		{
			...
			ASSERT(portrait, -1);
			ASSERT(output, -1);
			source = (unsigned char const *)portrait;
			destination = (unsigned char *)output;
			end = std::memchr(source, '\0', sizeof(*portrait));
			length = end ? (unsigned char const *)end-source : sizeof(*portrait);
			source_legacy = d2char_portrait_is_legacy(portrait);
			std::memset(destination, 0, D2CHARINFO_PORTRAIT_STORAGE_SIZE);

			if (legacy_100) {
				if (source_legacy) {
					std::memcpy(destination, source,
						std::min(length+1, (std::size_t)D2CHARINFO_PORTRAIT_STORAGE_SIZE));
					return 0;
				}
				destination[0] = 0x83;
				destination[1] = 0x80;
				std::memset(destination+2, D2CHARINFO_PORTRAIT_PADBYTE, 16);
				std::memcpy(destination+2, source+2, 11);
				destination[18] = source[13];
				std::memset(destination+19, D2CHARINFO_PORTRAIT_PADBYTE, 16);
				std::memcpy(destination+19, source+14, 11);
				destination[35] = source[25];
				std::memcpy(destination+36, source+26, 7);
				destination[43] = '\0';
				return 0;
			}

			if (!source_legacy) {
				std::memcpy(destination, source, D2CHARINFO_PORTRAIT_MODERN_SIZE);
				return 0;
			}
			destination[0] = 0x84;
			destination[1] = 0x80;
			std::memcpy(destination+2, source+2, 11);
			destination[13] = source[18];
			std::memcpy(destination+14, source+19, 11);
			destination[25] = source[35];
			std::memcpy(destination+26, source+36, 7);
			destination[33] = '\0';
			return 0;
		}
